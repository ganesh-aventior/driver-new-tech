            ...
            try:
                ...
                if provider_name.lower().replace(" ", "") == 'openweathermap':
                    ow = OpenWeatherMapRequests(api_key=weather_info_obj.client_id,
                                                ...
                                                expire_after=5 * 60)
                    data = ow.get_weather(lon=lat_lon[0], lat=lat_lon[1])  # display current weather data
                    ...
                    instance = serializer.save(weather=weather)
                elif provider_name.lower().replace(" ", "") == 'yahoo':
...
class DriverBoundaryViewSet(APIView):
...
class GetLatestRecordSchema(APIView):
    def post(self, request):
        with connections['default'].cursor() as cursor:
            ...
            # query = "select *  from data_recordcostconfig  where record_type_id::text = "+"'"+str(record_type_id)+"'"+" ORDER BY created DESC LIMIT 1"

            query = "select *  from grout_recordschema  where record_type_id ::text = " + "'" + str(
